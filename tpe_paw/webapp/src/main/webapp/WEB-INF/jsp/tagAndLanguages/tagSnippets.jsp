            ...
            <div class="flex-row flex-center title-container">
                ...
                <c:if test="${currentUser != null}">
                    <c:if test="${userRoles.contains('ADMIN')}">
                        <form:form action="${tagDeleteUrl}" class="form-container" method="post"
                            ...
                            <!-- Modal -->
                            <div class="modal fade" id="tag-snippet" tabindex="-1" role="dialog"
                                 aria-labelledby="tagDeletionModal" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title font" id="tagDeletionModal"><spring:message
                                                    code="modal.delete.title"/></h5>
                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                <span aria-hidden="true">&times;</span>
                                            </button>
                                        </div>
                                        <div class="modal-body modal-body-text fw-100">
                                            <spring:message var="modalTagDeleteMessage" code="modal.delete.tag.text"
                                                            arguments="${tag.name}"/>
                                            <c:out value="${modalTagDeleteMessage}"/>
                                        </div>
                                        <div class="modal-footer flex-end">
                                            <div class="flex-center">
                                                <div type="button" class="btn btn-secondary modal-close-button"
                                                     data-dismiss="modal"><c:out value="${modalCancel}"/></div>
                                                <form:checkbox class="hidden" path="delete" value="true"
                                                               id="tag-delete-button" onclick="updateForm(this)"/>
                                                <label for="tag-delete-button"
                                                       class="btn btn-primary purple-button  no-margin">
